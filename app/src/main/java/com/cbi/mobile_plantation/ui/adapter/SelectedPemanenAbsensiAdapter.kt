...
import com.cbi.mobile_plantation.ui.view.Absensi.WorkerRemovalListener
...
class SelectedPemanenAbsensiAdapter(private val removalListener: WorkerRemovalListener? = null) : RecyclerView.Adapter<SelectedPemanenAbsensiAdapter.ViewHolder>() {
    ...
    private fun removeWorker(position: Int, context: Context) {
        val removedWorker = selectedWorkers[position]
        selectedWorkers.removeAt(position)
        context.vibrate()

        AppLogger.d("Attempting to call onWorkerRemoved with id: ${removedWorker.id}, name: ${removedWorker.name}")
        AppLogger.d("Is removalListener null? ${removalListener == null}")

        // Notify the listener
        removalListener?.onWorkerRemoved(removedWorker.id, removedWorker.name)

        AppLogger.d("After calling onWorkerRemoved")
