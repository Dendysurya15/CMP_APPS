...
class CameraViewModel(private val cameraRepository: CameraRepository) : ViewModel() {
    ...
    fun takeCameraPhotos(context: Context, resultCode : String, imageView: ImageView, pageForm : Int, deletePhoto : View?, komentar :String, kodeFoto:String, featureName : String?, latitude:Double?= null, longitude:Double?= null, sourceFoto: String) {
        cameraRepository.takeCameraPhotos(context,resultCode, imageView, pageForm, deletePhoto,komentar, kodeFoto, featureName, latitude, longitude, sourceFoto)
    }
